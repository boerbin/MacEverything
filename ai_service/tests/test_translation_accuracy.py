...
ACCURACY_CASES = [
    ("最近下载的PDF", ["ext:pdf", "dm:"]),
    ("上个月修改的Word文档", ["ext:doc", "dm:lastmonth"]),
    ("桌面上的图片", ["path:Desktop", "pic:"]),
    ("大于500MB的视频", ["video:", "size:>500mb"]),
    ("今天创建的文件", ["dc:today"]),
    ("Python代码文件", ["ext:py"]),
    ("除了build目录的cpp文件", ["ext:cpp", "nopath:build"]),
    ("recent PDF files", ["ext:pdf", "dm:"]),
    ("large downloads", ["path:Downloads", "size:>"]),
    ("markdown files modified this week", ["ext:md", "dm:thisweek"]),
]


@pytest.mark.asyncio
@pytest.mark.parametrize("nl_input,expected_fragments", ACCURACY_CASES)
async def test_translation_accuracy(translator, nl_input, expected_fragments):
    ...
    for fragment in expected_fragments:
        assert fragment.lower() in query, (
            f"Expected '{fragment}' in translation of '{nl_input}', got: '{result.translated_query}'"
        )
